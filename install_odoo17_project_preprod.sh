...
create_logrotate() {
  cat <<EOF >/tmp/lgrotate
$PROJECT_PATH/odoo-server.log {
  daily
  rotate 14
  compress
  delaycompress
  ...
  notifempty
  copytruncate
  create 0644 $OE_USER $OE_USER
  su $OE_USER $OE_USER
}
EOF
  sudo mv /tmp/lgrotate /etc/logrotate.d/"${PROJECT_NAME}"_"$BRANCH_NAME"
  sudo chown root:root /etc/logrotate.d/"${PROJECT_NAME}"_"$BRANCH_NAME"
  sudo chmod 644 /etc/logrotate.d/"${PROJECT_NAME}"_"$BRANCH_NAME"
}
